...
const VoucherPage: React.FC = () => {
    ...
    const isFromAgentPortal = new URLSearchParams(location.search).get('from') === 'agent';
    const backLink = isFromAgentPortal ? '/agent/my-bookings' : '/my-bookings';
    const backLinkText = isFromAgentPortal ? 'Back to Agency Bookings' : 'Back to My Bookings';
    ...
    if (!booking) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen text-center p-4">
                ...
                <Link to={backLink} className="bg-primary text-white font-bold py-2 px-6 rounded-lg hover:bg-primary-800 transition">
                    {backLinkText}
                </Link>
            </div>
        );
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen text-center p-4 bg-gray-100">
                <div className="bg-white p-10 rounded-lg shadow-lg">
                    <h1 className="text-3xl font-bold text-yellow-600 mb-4">Voucher Not Available</h1>
                    <p className="text-gray-600 mb-2">This booking is currently <span className="font-bold">{booking.status}</span>.</p>
                    <p className="text-gray-600 mb-8">The voucher can only be downloaded or printed for confirmed bookings.</p>
                    <Link to={backLink} className="bg-primary text-white font-bold py-2 px-6 rounded-lg hover:bg-primary-800 transition">
                       {backLinkText}
                    </Link>
                </div>
        ...
        <div className="bg-gray-200 min-h-screen py-12 px-4">
            <div className="max-w-4xl mx-auto">
                <div className="flex flex-col sm:flex-row justify-between items-center mb-4 print:hidden gap-4">
                    <Link to={backLink} className="text-primary hover:underline">&larr; {backLinkText}</Link>
                        ...
                        <div className="p-8 md:p-10">
                            ...
                            <footer className="mt-12 pt-8 border-t text-sm text-gray-500">
                                ...
                                <ul className="list-disc list-inside space-y-1">
                                    <li>Please present this voucher upon check-in. A valid photo ID may be required.</li>
                                    <li>This booking is non-refundable unless otherwise stated in the booking policy.</li>
